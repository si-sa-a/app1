...
sequenceDiagram
    ...
    AI-->>PO: DOR自動チェック結果
    Note right of AI: ・✅ 達成項目<br/>・❌ 不足項目＋追記ガイド
    PO->>AI: 不足項目を追記・修正
    AI-->>PO: DORチェック再実行
...
```mermaid
flowchart LR
    A["やりたいことを<br/>自然言語で入力"] --> B["AI が PBI<br/>たたき台を生成"]
    B --> C["DOR<br/>自動チェック"]
    C --> D{DOR<br/>達成?}
    D -->|❌ 未達| E["不足項目を<br/>追記・修正"]
    E --> C
    D -->|✅ 達成| F["チームに<br/>レビュー依頼"]
    F --> G{未解決の<br/>質問あり?}
    G -->|あり| H["フィードバック<br/>反映・修正"]
    H --> C
    G -->|なし| I["リファインメント"]
    I --> J["DOR Ready<br/>最終判定"]
    J --> K["プランニング<br/>投入"]
